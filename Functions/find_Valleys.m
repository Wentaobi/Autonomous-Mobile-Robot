...
clearanceAngle=(2*(asin(robotW/(2*threshold)))*180/pi); %degrees
sectToClear=floor(clearanceAngle/3); %Number of sectors that need to be free
                                    %for the robot to pass through without
                                    %hitting.
                                    
b1=(find(diff(Hb)==-1)); %Find beginning of consecutive clear sectors
b1=b1+1;
b2=(find(diff(Hb)==1)); %Find end of consecutive clear sector
